function [ErrsWZD, dErrsdp, BB] = WJMODEL_BBFUN(pars, mdi, expdat, K, M, R, X0, Fv, L, Txyn, Qxyn, CFUN, Npatches, Nqp, opt)
...
%     [prev.Fxyn, dFxynstat, prev.dFxyndpar] = CFUN(prev.uxyn, pars);
    ...
    [prev.Fxyn, dFxynstat, prev.dFxyndpar] = CFUN(prev.uxyn, pars);
    prev.uxyn(:, 3) = prev.uxyn(:, 3)*0;
    prev.Fxyn(:, 3) = prev.Fxyn(:, 3)*0;
...
%             % Trapezoidal
%             Di = Di + (BB.Q(i)-Q0)*(qr(k)-qr(k-1))*qr(k)*abs(Xd(end));
%             dDdpi = dDdpi + (BB.Q(i)-Q0)*(qr(k)-qr(k-1))*qr(k)*dXdp(end,:);            
            
%             Di = Di + (BB.Q(i)-Q0)*wi(k)*qr(k)^2*abs(Xd(end));
%             dDdpi = dDdpi + (BB.Q(i)-Q0)*wi(k)*qr(k)^2*dXdp(end,:);
        end
