function [Q,T] = ZTE_ND2QP_LOC(MESH,No)
%ZTE_ND2QP_LOC Returns Matrix transforming nodal degrees of freedom to
...
%   			quadrature points to integrated nodal
    ...
    for e=1:MESH.Ne_Tri
        ...
        X   	= reshape(X,No^2,1);
        Y   	= reshape(Y,No^2,1);
        Ws      = reshape(Wx*Wy', No^2, 1);
        Nv      = TRI2D_SF_LOC([X Y],V);
        
        % Nodal to QP
        Q((e-1)*Nq+(1:Nq),nds) = Nv;
        
        % QP to integrated Nodal
        T(nds,(e-1)*Nq+(1:Nq)) = T(nds,(e-1)*Nq+(1:Nq))+Nv'.*Ws';
    end
    ...
    for e=1:MESH.Ne_Quad
        ...
        T(nds,(MESH.Ne_Tri+e-1)*Nq+(1:Nq)) = T(nds,(MESH.Ne_Tri+e-1)*Nq+(1:Nq))+Jd'.*Nv'.*Ws';
    end
end