...
if size(EA,2)==2
    c1 = cos(EA(1,1));	s1 = sin(EA(1,1));
    c2 = cos(EA(2,1));	s2 = sin(EA(2,1));
    c3 = cos(EA(3,1));	s3 = sin(EA(3,1));
    % Calculate Matrix
    R = [c1*c2           -c2*s1          s2
     c3*s1+c1*s2*s3  c1*c3-s1*s2*s3  -c2*s3
     s1*s3-c1*c3*s2  c3*s1*s2+c1*s3  c2*c3]';
 
    c1 = cos(EA(1,2));	s1 = sin(EA(1,2));
    c2 = cos(EA(2,2));	s2 = sin(EA(2,2));
    c3 = cos(EA(3,2));	s3 = sin(EA(3,2));
    % Calculate Matrix
    S = [c1*c2           -c2*s1          s2
     c3*s1+c1*s2*s3  c1*c3-s1*s2*s3  -c2*s3
     s1*s3-c1*c3*s2  c3*s1*s2+c1*s3  c2*c3]'; 
...
elseif size(CV,2)==2
    for i = [1 4 5 8]
        verts(1:3,i) = R*[x(i)+0.5*SL(1);y(i);z(i)]+CV(:,1);
    end
    for i = [2 3 6 7]
        verts(1:3,i) = S*[x(i)-0.5*SL(1);y(i);z(i)]+CV(:,2);
    end
    CV = mean(CV, 2);
end
%% Draw Patch Object
