function [varargout] = IQSMA_CALC(pars, modelmats, expdat, Prestress, opts)
...
%     X0 = modelmats.K\(Prestress*modelmats.Fv);
    X0 = load('statsol.dat');
    prev.uxyntxyn = sparse(size(modelmats.QuadMats.Q,1),6);
    
    [Xstat, ~, ~, ~, dR0] = fsolve(@(X) NLRES(NLFUN, [X; 0], modelmats.K, ...
        modelmats.L, Prestress*modelmats.Fv, modelmats.Fv*0, ...
        prev), X0, opt);
    [Px, Py, Pn] = CFUNC(modelmats.QuadMats.Q*reshape(modelmats.L(1:(3*modelmats.MESH.Nn),:)*Xstat,3,modelmats.MESH.Nn)',prev.uxyntxyn);
    Pstat = reshape([Px Py Pn]', size(modelmats.QuadMats.Q,1)*3, 1);
    dlmwrite('statsol.dat', Xstat, 'precision', '%10e');
    
    prev.uxyntxyn = [modelmats.QuadMats.Q*reshape(modelmats.L(1:(3*modelmats.MESH.Nn),:)*Xstat,3,modelmats.MESH.Nn)' Px Py Pn];
...
%     parfor i=1:opts.Na
    for i=1:opts.Na
        [HYST, BB, ~, ~] = STEPPEDHYSTERESIS(NLRESIDFUNC, CFUNC, ...
        ...
        [D(i),dx(:,i),dy(:,i),dn(:,i)] = modelmats.DISSFUNC(modelmats.L(1:(3*modelmats.MESH.Nn),:)*(HYST.U-Xstat), ...
