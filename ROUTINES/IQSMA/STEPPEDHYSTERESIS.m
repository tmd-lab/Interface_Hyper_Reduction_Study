function [HYST,BACKBONE,UNLOAD,RELOAD] = STEPPEDHYSTERESIS(nlresfun, contactfun, ...
                                             Xstart, Alphamax, Np, ...
                                             MESH, L, opt, QuadMats, prev, varargin)
...
%         opt.FunctionTolerance = (ALPHAS_BB(i)+eps)^4;
        Ubb(:,i)      = fsolve(@(X) nlresfun(X,ALPHAS_BB(i),prev), Xguess, ...
                          opt);
        Ueph          = L*Ubb(:,i);
        [Ptx,Pty,Pn]  = contactfun(QuadMats.Q*reshape(Ueph(1:(3*MESH.Nn)),3,MESH.Nn)',prev.uxyntxyn);
...
%             Xguess 	  = Ubb(:,i) + dXda*(ALPHAS_BB(i+1)-ALPHAS_BB(i));
    ...
    %% Unloading
    prev.uxyntxyn(:,1:3) = QuadMats.Q*reshape(Ueph(1:(3*MESH.Nn)),3,MESH.Nn)';
...
%         opt.FunctionTolerance = (ALPHAS_UL(i)+eps)^2;
        Uul(:,i) = fsolve(@(X) nlresfun(X,ALPHAS_UL(i),prev), Xguess, ...
                          opt);
        Ueph 	 = L*Uul(:,i);
        [Ptx,Pty,Pn] = contactfun(QuadMats.Q*reshape(Ueph(1:(3*MESH.Nn)),3,MESH.Nn)',prev.uxyntxyn);
...
%         opt.FunctionTolerance = (ALPHAS_RL(i)+eps)^2;
        Url(:,i) = fsolve(@(X) nlresfun(X,ALPHAS_RL(i),prev), Xguess, ...
                          opt);
        Ueph 	 = L*Url(:,i);
        [Ptx,Pty,Pn] = contactfun(QuadMats.Q*reshape(Ueph(1:(3*MESH.Nn)),3,MESH.Nn)',prev.uxyntxyn);
    ...
    BACKBONE.U	= Ubb;
    BACKBONE.P  = Pbb;
    BACKBONE.F	= kron(QuadMats.T,eye(3))*Pbb;
    BACKBONE.A  = ALPHAS_BB;

    UNLOAD.U	= Uul;
    UNLOAD.P    = Pul;    
    UNLOAD.F	= kron(QuadMats.T,eye(3))*Pul;
    UNLOAD.A    = ALPHAS_UL;
    
    RELOAD.U	= Url;
    RELOAD.P    = Prl;    
    RELOAD.F	= kron(QuadMats.T,eye(3))*Prl;
    RELOAD.A    = ALPHAS_RL;
    ...
    HYST.F  = kron(QuadMats.T,eye(3))*HYST.P;               
