function [Qx,Qy,Tx,Ty] = ZTE_ND2QPD(MESH,No)
...
%   			quadrature points to integrated nodal
    ...
    for e=1:MESH.Ne_Tri
        ...
        Tx(nds,(e-1)*Nq+(1:Nq)) = Tx(nds,(e-1)*Nq+(1:Nq))+Jd'.* ...
            (Qx((e-1)*Nq+(1:Nq),nds))'.*Ws';
        Ty(nds,(e-1)*Nq+(1:Nq)) = Ty(nds,(e-1)*Nq+(1:Nq))+Jd'.* ...
            (Qy((e-1)*Nq+(1:Nq),nds))'.*Ws';
    end
    ...
    for e=1:MESH.Ne_Quad
        ...
        Tx(nds,(MESH.Ne_Tri+e-1)*Nq+(1:Nq)) = Tx(nds,(MESH.Ne_Tri+e- ...
                                                     1)*Nq+(1:Nq))+ ...
            Jd'.*(Qx((MESH.Ne_Tri+e-1)*Nq+(1:Nq),nds))'.*Ws';
        Ty(nds,(MESH.Ne_Tri+e-1)*Nq+(1:Nq)) = Ty(nds,(MESH.Ne_Tri+e- ...
                                                     1)*Nq+(1:Nq))+ ...
            Jd'.*(Qy((MESH.Ne_Tri+e-1)*Nq+(1:Nq),nds))'.*Ws';
    end
end