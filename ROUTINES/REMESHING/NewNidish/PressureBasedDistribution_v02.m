...
SegNumEle=zeros(1,5);
for N_Segments=1:5 % Determine number of Segements which should be used
    
    scaleFactor=(scale.min +scale.max)/2;
    [New_Nodes,~,idx_holeBound,~] = PressureBasedNodePlacement_v03(x_red,y_red,x,y,preload_red,idx_reduced,bolt_idx,corners_idx,N_Segments,scaleFactor,Setting);
...
scaleFactor=(scale.min +scale.max)/2;
[New_Nodes,~,idx_holeBound,dx] = PressureBasedNodePlacement_v03(x_red,y_red,x,y,preload_red,idx_reduced,bolt_idx,corners_idx,N_Segments,scaleFactor,Setting);
[Elements_idx,qualityflag] = MeshGeometryBased(New_Nodes,Setting.MeshMethod, Setting.MeshMethodLimit,idx_holeBound);
