...
copt.lspci = [1 2 3];  % [Fs Kt Kn]  all log scale

copt.x.T{1} = [PatchAreas' zeros(Npatches, 2)];  % Fs
copt.x.T{2} = [zeros(Npatches,1) PatchAreas' zeros(Npatches, 1)];  % Kt

copt.y.T{1} = [PatchAreas' zeros(Npatches, 2)];  % Fs
...
%some arbitary displacements from running below steps in debug mode and
%pulling numbers. 

lpars = pars(:);
lpars(copt.lspci) = 10.^(lpars(copt.lspci));

u_slip = lpars(1) / lpars(2);

% %%Add two test cases one for fully stuck and one for fully slipped. 
uxyn = 1.0e-12 * [-0.000154120577451  -0.000022253424982  -0.010028747758349
                  -0.000326602422792   0.000043251001960   0.168616763073667
                  -0.000135580797286   0.000110378529744   0.158927751207691
                  -0.000012461805329   0.000010715084769   0.168695241924421
                   0.000383621732913   0.000055344469513  -0.010167365657410];
               
uxyn1 = uxyn;
uxyn1(:, 1:2) = [u_slip/2        u_slip/2;
                 -u_slip/2       -u_slip/2;
                 -u_slip/2       u_slip/2;
                 u_slip/2        -u_slip/2;
                 1.5*u_slip      -u_slip/2];
...
uxyn2 = uxyn;
uxyn2(:, 1:2) = [1.5*u_slip        1.5*u_slip;
                 -1.5*u_slip       -1.5*u_slip;
                 -1.5*u_slip       1.5*u_slip;
                 1.5*u_slip        -1.5*u_slip;
                 u_slip/2          -1.5*u_slip];

final_diff = CHECK_JACOBIANS(CFUN, uxyn, pars)
final_diff1 = CHECK_JACOBIANS(CFUN, uxyn1, pars)
final_diff2 = CHECK_JACOBIANS(CFUN, uxyn2, pars)
