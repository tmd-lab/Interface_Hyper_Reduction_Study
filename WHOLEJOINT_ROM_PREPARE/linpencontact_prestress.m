...
figure(1); clf();
subplot(2,1,1)
SHOW2DMESH(MESH.Nds, [], MESH.Quad, Qm\Tn, -1, -100); % colorbar('north'); 
colormap(jet(100)); axis equal; axis off; title('Normal Pressure')
subplot(2,1,2)
SHOW2DMESH(MESH.Nds, [], MESH.Quad, V(3:3:MESH.Nn*3,1), -1, -100); % colorbar('southoutside'); 
colormap(jet(100)); axis equal; axis off; title('Normal displacement')

save(sprintf('./DATS/statsol_P%d_S%.2f.mat',Prestress,log10(sint)), 'Xs', 'Qm', 'Tm', 'Tx', 'Ty', 'Tn', 'dR0', 'MESH','sint', 'V')